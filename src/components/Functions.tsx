...
export const Functions = () => {
  ...
  const getImageClass = useCallback(
    (index: number) => {
      const base =
        "h-[600px] max-lg:h-[75vh] transition-all duration-300 hover:opacity-100 lg:scale-100 lg:hover:scale-105 cursor-pointer";
  ...
  return (
    <>
      ...
      >
        <button className="h-[590px] overflow-hidden" ref={pic1Ref} onClick={() => handleSelect(0)}>
          <div id="img1" className="relative -top-10" />
          <img className={getImageClass(0)} src={push_workout} alt="" />
        </button>
        <button className="h-[590px] overflow-hidden" ref={pic2Ref} onClick={() => handleSelect(1)}>
          <img className={getImageClass(1)} src={graph} alt="graph" />
          <div id="img2" />
        </button>
        <button className="h-[590px] overflow-hidden" ref={pic3Ref} onClick={() => handleSelect(2)}>
